...
class KRAReview extends Component {
  ...
  loadKRAs = () => {
    ...
    rhythmus_api("kra-review", {teammate_id : userid })
      .then((data) => {
        let teammate = data;
  ...
  render() {
    ...
    if (teammate && teammate.name && month && year) {
      ...
      let body = "";
      let viewBtn = (
        <Button className={classes.prevBtn} onClick={this.onViewKRA} disabled={this.state.saving}>
          <IconBack /> Back
        </Button>
      );
      if (!this.state.isEditing && !this.state.isSubmitting) {
        viewBtn = "";
        body = (
          <KRAReviewViewer review={review} classes={classes} teammate={teammate}></KRAReviewViewer>
        );
      }
      ...
      if (this.state.isEditing) {
        ...
        editBtn = (
          <Button className={classes.prevBtn} onClick={this.onEditKRA} disabled={this.state.saving}>
            <IconEdit /> Edit
          </Button>
        );
      }
          ...
          <Grid container>
            ...
            <Grid container justify="flex-end">
              <ButtonGroup size="small" aria-label="small button group">
                <Button
                  className={classes.prevBtn}
                  onClick={this.onChooseTeammatePrevMonth}
                  disabled={this.state.saving}
                  title={prevMonthLabel}
                >
                  <IconBack /> {prevMonthLabel}
                </Button>
                <Button
                  className={classes.nextBtn}
                  onClick={this.onChooseTeammateNextMonth}
                  disabled={this.state.saving}
                  title={nextMonthLabel}
                >
                  {nextMonthLabel} <IconNext />
                </Button>
                {closeBtn}
              </ButtonGroup>
            </Grid>
          ...
          <form className={classes.container} noValidate autoComplete="off">
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                <h2>
                  {teammate.name} for {m[month - 1]}, {year}
                </h2>
                {totalContent}
              </Paper>
            </Grid>
