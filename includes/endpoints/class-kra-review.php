...
class KRA_Review extends Abstract_Endpoint {
	...
	public function read( $request ) {
		...
		$sql = $wpdb->prepare(
			"SELECT rt.id, fname, lname, year, month, total, submitted, reviewed, topics, 
        ...
        WHERE rt.id = %d and is_active = 1 ORDER BY year DESC, month DESC",
			$teammate_id
		);
		...
		foreach ( $results as $row ) {
			if ( ! array_key_exists( 'userid', $teammate ) || ! $teammate['userid'] ) {
			...
			$months[ $key ] = array(
				'total'        => $row->total,
				'reviewed'     => 1 === $row->reviewed,
				'submitted'    => 1 === $row->submitted,
